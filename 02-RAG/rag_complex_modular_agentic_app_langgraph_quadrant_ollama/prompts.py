...
Each prompt corresponds to one node in the LangGraph workflow:

    - Conversation summary: extracts key topics from chat history
    - Query analysis:       rewrites unclear queries, optionally splits multi-part ones
    - RAG agent:            forces retrieval before answering, retries once on failure
    - Aggregation:          merges multiple sub-answers into a final response

Key behaviors enforced via these prompts:
    - Query rewriting uses prior context only when truly needed
    - Agent must search before answering (no hallucination)
    - One retry with reformulated query if initial retrieval is empty
    - Source attribution at the end of the final response

Replace this module to retune behavior without touching graph wiring or nodes.
"""


def get_conversation_summary_prompt() -> str:
    """Prompt for the conversation summarizer node."""
...
def get_query_analysis_prompt() -> str:
    """Prompt for the query analyzer / rewriter node."""
...
def get_rag_agent_prompt() -> str:
    """Prompt for the inner ReAct retrieval agent."""
...
def get_aggregation_prompt() -> str:
    """Prompt for the response aggregator node."""
